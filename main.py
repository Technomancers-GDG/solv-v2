...
def resolve_logistics_graph(
    ...
    if not db_graph.nodes or not db_graph.edges:
        raise HTTPException(
            status_code=400,
            detail="No logistics graph provided and no persisted nodes/edges are available",
        )
    return db_graph
    ...
    *,
    ...
    try:
        ...
        if apply_predictions:
            # Dry-run routes give the prediction engine candidate port/rail nodes
            # and route signatures. The final Dijkstra run uses adjusted weights.
            preliminary_routes = multimodal_graph_engine.compute_routes(
                resolved_graph,
                origin_node_id,
                destination_node_id,
                weights,
            ...
            if preliminary_routes:
                predictions = logistics_prediction_engine.predict(
                    ...
                    route_options=preliminary_routes,
                    port_context=port_context,
                    rail_context=rail_context,
                    weather_context=weather_context,
                    shipment_context=shipment_context,
                )
                adjusted_graph = logistics_prediction_engine.adjust_graph_for_predictions(
                    graph=resolved_graph,
        ...
        routes = multimodal_graph_engine.compute_routes(
            routing_graph,
            origin_node_id,
            destination_node_id,
            weights,
...
@app.post("/api/fusion/graph", response_model=DataFusionResponse)
@app.post("/fusion/graph", response_model=DataFusionResponse)
def fuse_logistics_graph(
    payload: DataFusionRequest,
    session: Session = Depends(get_session),
) -> DataFusionResponse:
    """Normalize warehouse, port, railhead, mine, road, rail, and water feeds."""
    try:
...
@app.post("/api/compute-routes", response_model=ComputeRoutesResponse)
@app.post("/compute-routes", response_model=ComputeRoutesResponse)
def compute_routes(
    payload: ComputeRoutesRequest,
    session: Session = Depends(get_session),
) -> ComputeRoutesResponse:
    """Run Dijkstra over the multimodal graph with cost/time/risk scoring."""
...
def predict_logistics(
    ...
    """Predict port delay, rake availability, and route risk using mock rules."""
    ...
    if not route_options:
        if not payload.origin_node_id or not payload.destination_node_id:
            raise HTTPException(
                status_code=400,
                detail="Provide route_options or origin_node_id and destination_node_id",
            )
...
@app.post("/api/decide-route", response_model=DecisionResponse)
@app.post("/decide-route", response_model=DecisionResponse)
def decide_route(payload: DecisionRequest) -> DecisionResponse:
    """Combine route options and predictions, then auto-switch if thresholds breach."""
    try:
...
@app.post("/api/assign-route", response_model=AssignmentResponse)
@app.post("/assign-route", response_model=AssignmentResponse)
def assign_route(
    payload: AssignRouteRequest,
    session: Session = Depends(get_session),
) -> AssignmentResponse:
    """Assign a selected multimodal route to a shipment, driver, and vehicle."""
...
def reroute_shipment(
    ...
    """Compute and execute a reroute for an assigned shipment."""
    ...
    origin_node_key = (
        payload.current_location_node_id
        or shipment.current_location_node_key
        or shipment.origin_node_key
    )
    ...
    if not route_options:
        _, _, route_options, _ = compute_logistics_routes(
            session=session,
            graph=graph,
            origin_node_id=origin_node_key,
    ...
    previous_route = (
        session.get(LogisticsRoute, shipment.current_route_id)
        if shipment.current_route_id is not None
        else None
    )
    previous_route_option = (
        logistics_execution_service.route_option_from_model(previous_route)
        if previous_route is not None
        else None
    )
    improvement_score = (
        logistics_decision_engine.improvement_score(previous_route_option, decision.recommended_route)
        if previous_route_option is not None
        else float("inf")
    )
    if previous_route_option is not None and improvement_score <= payload.improvement_threshold:
        kept_reason = (
            f"{decision.reasoning} Kept current route because improvement score "
            f"{improvement_score:.2f} did not exceed threshold {payload.improvement_threshold:.2f}."
        )
        decision = decision.model_copy(
            update={
                "recommended_route": previous_route_option,
                "reasoning": kept_reason,
                "switched": False,
            }
        )
        return RerouteResponse(
            shipment_id=shipment.id,
            previous_route=previous_route_option,
            new_route=previous_route_option,
            decision=decision,
            status="kept_current_route",
            rerouted=False,
...
@app.post("/api/telemetry/simulate", response_model=TelemetrySimulationResponse)
@app.post("/telemetry/simulate", response_model=TelemetrySimulationResponse)
def simulate_logistics_telemetry(payload: TelemetrySimulationRequest) -> TelemetrySimulationResponse:
    """Estimate fuel, emissions, and delay for an executed route."""
    return telemetry_simulation_service.simulate(
        route=payload.route,
        payload_tons=payload.payload_tons,
